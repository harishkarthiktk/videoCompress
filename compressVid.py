...
'''
The script converts the {{SUPPORTED FORMATS}} to .mp4 format with optimal compression.
It uses gpu acceleration if possible, and skips a file if _conv.mp4 already exists for the same file.
'''
...
def convert_video(file, gpu_type, progress_bar):
    ...
    ffmpeg_cmd = [
        "ffmpeg", "-y", "-i", file, "-vf",
        "scale='if(gt(iw,1920),1920,if(lt(iw,1280),1280,iw))':'if(gt(ih,1080),1080,if(lt(ih,720),720,ih))'"
    ]
...
def main():
    ...
    if arg:
        print(f"Runing script in path: {arg}")
    else:
        print("No path was provided, hence running from script's root.")
